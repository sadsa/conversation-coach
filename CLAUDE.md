...
- **Onboarding collapsed to the language pick** (`app/onboarding/page.tsx`): `?step=0` (or no step) renders the language picker — the entire required wizard. Confirming the language pushes straight to `/?welcome=true` (peak-end beat on home). `?step=2` is the standalone WhatsApp share illustration, rendered via `<OnboardingStep>` (`totalSteps=1` so the dot row hides, no Back button — there's nowhere to return to within the wizard). It's reached from the home page's "Already recorded a voice note?" Share CTA below the Practice card. Any other step value (legacy `?step=1` hub URLs, out-of-range) clamps to step 2 so stale bookmarks still land somewhere meaningful. `revisit=true` is kept as forward-compat — if a Settings re-entry returns, exits will land on `/settings`; currently nothing sets it. The hub (`OnboardingHub`), the empty-state `DashboardOnboarding` "Revisit tutorial" link, and the Settings → Help section were all retired together — the home Share CTA is now the only entry point.
