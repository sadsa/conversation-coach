...
- **Vocabulary page** (`/vocabulary`): `VocabularyClient` → `VocabularyList` orders saved items into study-status buckets (Due for review → To study → Studied), each headed by a state dot (amber/violet/emerald, reused from the annotation-state vocabulary) + label + count; empty buckets are omitted. Session provenance is demoted to the review sheet's "From …" link, not a list heading. `now` re-evaluates buckets ~once a minute so items promote into "Due" on a long-open page. Empty handling distinguishes a filter-no-match state ("Clear filters" recovery, via `filterActive` + `onClearFilters`) from the first-run onboarding empty state. Within each bucket `WriteList` renders the rows; mark-studied / move-back / delete live in the per-row ⋮ menu (`RowActionsMenu`). Delete is optimistic with 5s undo — `DELETE` fires after the timer expires.
