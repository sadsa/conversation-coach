...
components/
  ...
  MethodologyEyebrow.tsx          # "Practise → Review → Study" eyebrow row — appears on /, /review, /write so the user can always orient inside the methodology
...
- **Call mode = learner-answers-first, not agent-opens** (`lib/persona.ts` + `components/PracticeClient.tsx`): Real phone-call metaphor. After tapping Answer the agent stays silent until the learner greets ("Hello, Josh speaking" / "Hola, soy Josh"); the persona's `opener` field is then delivered as a REPLY to that greeting (model is told to adapt it lightly — e.g. use the learner's name if given). The legacy `__START_CALL__` clientContent trigger / auto-speak path was removed — call mode no longer passes `openingLine` to `connect()`. The mechanism stays in `lib/voice-agent.ts` as an opt-in primitive for any future surface that genuinely needs the agent to speak first. The active screen surfaces a quiet "Your turn — say hello" cue above the audio dots until the first user turn lands so the silence reads as expected rather than broken. The persona writer prompt was retuned to write openers as greeting-responses rather than cold-opens; both writer-generated and templated-fallback openers follow this pattern. The dead `ringing` state (a brief auto-progress beat that used to gap-fill the reroll path) was removed when reroll started routing through `incoming` — every new caller now goes through the same Answer / Decline gate.
...
- **Practise-as-home + methodology vocabulary**: The methodology is **Practise → Review → Study**, and the URL layout reflects it. `/` is **Practise** (mode picker via `<PractiseClient>` — call / chat / share-a-voice-note), NOT a dashboard. `/review` is the **Review** inbox of recorded conversations (the dashboard moved here when the home was repurposed). `/write` keeps its URL for stability but is labelled **"Study"** in the nav and brand vocabulary. `<MethodologyEyebrow>` renders a "Practise → Review → Study" row beneath the H1 on all three surfaces — current pillar in accent (non-interactive `<span>`), other two as `<Link>`s — so deep-link arrivals always know where they are. Bottom nav order matches the methodology. There is no `/practice` route: the active voice session UI (`<PracticeClient>`) mounts in place on `/` when the user taps a mode door and unmounts back to the doors view on discard, so the home is the only entry AND exit for live sessions.
