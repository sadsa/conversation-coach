...
- **Onboarding is URL-driven** (`app/onboarding/page.tsx`): `?step=0` is the language picker (one-time gate). `?step=1` is the **hub** — `<OnboardingHub>`'s two-card decisive choice between Practice (links to `/practice`) and Share (links to `?step=2`). `?step=2` is the WhatsApp share illustration deep-dive, rendered via `<OnboardingStep>` (with `totalSteps=1` so the dot row hides). `&revisit=true` swaps "Skip" for "Close" and routes exits back to `/settings` instead of `/`. Settings → Help has two entries: "Show me the tutorial again" → `?step=1&revisit=true` (the hub) and "Share from WhatsApp" → `?step=2&revisit=true` (the share deep-dive). Upload-from-file is gone as a primary input, so the old upload tutorial step (and `UploadIllustration`) was removed.
