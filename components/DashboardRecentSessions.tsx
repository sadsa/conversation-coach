...
// "Show fewer" toggle when there are more. Avoids creating a separate
// /sessions index route while keeping the dashboard a comfortable length
// for users with many recordings.
//
...
export function DashboardRecentSessions({ sessions, onDeleted }: Props) {
      ...
      <div className="space-y-6">
        {groups.map(group => (
          <div key={group.bucket} className="space-y-2">
            ...
            <SessionList sessions={group.items} onDeleted={onDeleted} />
          </div>
        ))}
      </div>
