...
import type { SessionReviewState } from '@/lib/types'
...
export function StudyPrompt({ count, reviewState, onLaunchStudy, onFinishReview }: Props) {
  const { t } = useTranslation()
  const prefersReducedMotion = useReducedMotion()

  const status = count > 0
    ? (count === 1 ? t('transcript.phraseSaved') : t('transcript.phrasesSaved', { n: count }))
    : reviewState === 'nothing_kept'
    ? t('transcript.emptyNothingKept')
    : t('transcript.emptyPartial')
